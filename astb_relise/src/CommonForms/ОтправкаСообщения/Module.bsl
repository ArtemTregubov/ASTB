...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+++АСТБ_Горюшин_Алексей_20401
	//ТелоПисьма.УстановитьHTML(ТекстВHTML("<наберите здесь текст сообщения>"), ВложенияДляПисьма);
	//было
	//ТелоПисьма.УстановитьHTML(ТекстВHTML(Параметры.Тело), ВложенияДляПисьма);
	//---АСТБ_Горюшин_Алексей_20401
	
	ТелоПисьма.УстановитьHTML(ТекстВHTML("<опишите, пожалуйста, свою проблему максимально подробно и с примерами>"), ВложенияДляПисьма);
	...
	//+++АСТБ_Горюшин_Алексей_20401
	ЗаписатьСлужебнуюПодпись(Параметры.Тело);
	//---АСТБ_Горюшин_Алексей_20401
...
Процедура ОтправитьПисьмо()
	...
	//+++АСТБ_Горюшин_Алексей_20401
	ДополнитьТелоПисьмаСлужебнойПодписью();
	//---АСТБ_Горюшин_Алексей_20401
...
Функция ПоляЗаполненыКорректно()
	...
	Если НЕ ЗначениеЗаполнено(ТемаПисьма) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ru = 'Необходимо заполнить тему письма'"), , "ТемаПисьма");
		Результат = Ложь;
	КонецЕсли;
	//АсТБ_Alexey_32345_********************************************************************
...
//+++АСТБ_Горюшин_Алексей_20401
&НаСервере
Процедура ЗаписатьСлужебнуюПодпись(ЗНАЧ ТекстТела)
	
	//АсТБ_Alexey_32345_********************************************************************
	Если СтрНайти(ТекстТела, "<опишите, пожалуйста, свою проблему максимально подробно и с примерами>") > 0 Тогда
		ТекстТела = СтрЗаменить(ТекстТела, "<опишите, пожалуйста, свою проблему максимально подробно и с примерами>" 	+ Символы.ПС, "");
		//АсТБ_Alexey_32345_********************************************************************
		ПустоеВложение = Новый Структура;
		СлужебнаяПодпись.УстановитьHTML(ТекстВHTML(ТекстТела),ПустоеВложение);
	КонецЕсли;
	
КонецПроцедуры //---АСТБ_Горюшин_Алексей_20401

//+++АСТБ_Горюшин_Алексей_20401
&НаСервере
Процедура ДополнитьТелоПисьмаСлужебнойПодписью()
	
	СлужебнаяПодписьТекст = "" + Символы.ПС + СлужебнаяПодпись.ПолучитьТекст();
	ЗакладкаКонецПисьма = ТелоПисьма.ПолучитьЗакладкуКонца();
	ТелоПисьма.Вставить(ЗакладкаКонецПисьма, СлужебнаяПодписьТекст);
	
КонецПроцедуры //---АСТБ_Горюшин_Алексей_20401

//+++АСТБ_Горюшин_Алексей_20401
// Добавлен реквизит формы СлужебнаяПодпись
// На форму добавлено поле СлужебнаяПодпись
//---АСТБ_Горюшин_Алексей_20401