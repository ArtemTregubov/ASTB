...
Процедура Заполнить(Команда)
	//АсТБ_Alexey_46794_********************************************************************
	Если Сервер = "" Тогда
		ПолныйАдрес = НСтр("ru = 'ftps://логин:пароль@сервер:порт/каталог/'");
	Иначе
		Если Логин = "" Тогда
			ПолныйАдрес = "ftps://"+ Сервер +":"+ Формат(Порт, "ЧН=21; ЧГ=0") + Каталог;
		Иначе
			ПолныйАдрес = "ftps://"+ Логин +":"+ ?(ЗначениеЗаполнено(Пароль), ПарольСкрыт(), "") +"@"+ Сервер +":"+ Формат(Порт, "ЧН=0; ЧГ=0") + Каталог;
		КонецЕсли;
	КонецЕсли;
	//АсТБ_Alexey_46794_********************************************************************
...
Процедура ВидимостьДоступность(Форма, Изменения = "")
	...
	КонецЕсли;
	//АсТБ_Alexey_46794_********************************************************************
	Форма.СерверИКаталог = "ftps://"+ Форма.Сервер + Форма.Каталог;
	//АсТБ_Alexey_46794_********************************************************************
КонецПроцедуры
