...
//
Функция УКО_КодНаВстроенномЯзыке_Значение(Значение, ДополнениеРезультата = Неопределено, Язык = Неопределено, ДобавлятьКомментарии = Истина) Экспорт
		...
		Для Каждого СтрокаТЗ Из Значение Цикл 
			...
			Для Каждого Колонка Из Значение.Колонки Цикл 
				СтрокиКода.Добавить(СтрШаблон(НСтр("ru = 'НоваяСтрока.%1 = %2;'; en = 'NewRow.%1 = %2;'", Язык),
												Колонка.Имя, УКО_КодНаВстроенномЯзыке_Значение(СтрокаТЗ[Колонка.Имя], , Язык, ДобавлятьКомментарии)));
			КонецЦикла;
		КонецЦикла;
...
Функция УКО_ОбщегоНазначенияКлиентСервер_ЭтоПростойТип(Тип)
	...
	Результат = Тип = Тип("Null") ИЛИ Тип = Тип("Неопределено") ИЛИ Тип = Тип("Число") ИЛИ Тип = Тип("Строка") ИЛИ Тип = Тип("Дата") ИЛИ Тип = Тип("Булево") 
				...
				ИЛИ Тип = Тип("ВидДвиженияБухгалтерии") ИЛИ Тип = Тип("ВидДвиженияНакопления") ИЛИ Строка(Тип) = "НеизвестныйОбъект";
