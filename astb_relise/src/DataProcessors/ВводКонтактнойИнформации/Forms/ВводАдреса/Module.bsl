...
Процедура АдресНаДатуПриИзменении(Элемент)
	...
	Если Не ВводНовогоАдреса Тогда
		...
		ЗначенияПолей = ?(Результат.ТекущаяСтрока <> Неопределено, Результат.ТекущаяСтрока.ЗначенияПолей, "");
		Тип = ?(Результат.ТекущаяСтрока <> Неопределено, Результат.ТекущаяСтрока.Тип, ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Адрес"));
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС)
	...
	Иначе
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(Результат.ДействуетС) 
				И ПустаяСтрока(Результат.ТекущаяСтрока.ЗначенияПолей) Тогда
					Элементы.ГруппаИсторическийАдрес.Видимость = Ложь;
	...
	Если ЗначениеЗаполнено(АдресНаДату) Тогда
		Элементы.АдресНаДату.ФорматРедактирования = "";
	Иначе
		Элементы.АдресНаДату.ФорматРедактирования =  "ДФ='""начала учета""'";
	КонецЕсли;
...
Функция ОпределитьДатуДействия(ДействуетС, История)
	...
	Минимум = -1;
	Минимум2 = Неопределено;
	Для каждого СтрокаИстория Из История Цикл
		Дельта = СтрокаИстория.ДействуетС - ДействуетС;
		Если Дельта <= 0 И (Минимум2 = Неопределено ИЛИ Дельта > Минимум2) Тогда
