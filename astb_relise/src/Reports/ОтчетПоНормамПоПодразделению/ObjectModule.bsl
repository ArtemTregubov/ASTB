...
Функция ПолучитьТаблицуНормПодразделения(ДатаАнализа,Организация,Подразделение = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ЕСТЬNULL(ПОДСТРОКА(УточненныеОснованияНормВыдачи.Основание.Владелец.Наименование, 6, 15), """") КАК НомерТОН,
	               |	ЕСТЬNULL(ПОДСТРОКА(УточненныеОснованияНормВыдачи.Основание.Наименование, 3, 20), """") КАК НомерПриложенияТОН
	               |ИЗ
	               |	РегистрСведений.УстановленныеНормыВыдачиСИЗ.СрезПоследних(&ДатаАнализа, Организация = &Организация
	               ...
	               |		ПО УстановленныеНормыВыдачиСИЗСрезПоследних.Организация = УточненныеОснованияНормВыдачи.Организация
	               |			И УстановленныеНормыВыдачиСИЗСрезПоследних.Подразделение = УточненныеОснованияНормВыдачи.Подразделение
	               |			И УстановленныеНормыВыдачиСИЗСрезПоследних.Должность = УточненныеОснованияНормВыдачи.Должность
	               |			И УстановленныеНормыВыдачиСИЗСрезПоследних.НормаВыдачи = УточненныеОснованияНормВыдачи.НормаВыдачи
	               |			И УстановленныеНормыВыдачиСИЗСрезПоследних.РабочееМесто = УточненныеОснованияНормВыдачи.РабочееМесто
	               |ГДЕ
...
Процедура ДобавлениеУточненныхОснованийНорм(ТаблицаНорм)
	...
	|	ТаблицаНорм.НомерТОН КАК НомерТОН,
	|	ТаблицаНорм.НомерПриложенияТОН КАК НомерПриложенияТОН
	...
	|	КОНЕЦ КАК Основание,
	|	ВТ_ТаблицаНорм.НомерТОН КАК НомерТОН,
	|	ВТ_ТаблицаНорм.НомерПриложенияТОН КАК НомерПриложенияТОН
	|ИЗ
