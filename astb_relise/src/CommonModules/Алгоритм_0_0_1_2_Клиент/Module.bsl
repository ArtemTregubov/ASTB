...
Процедура ВыдачаСИЗ_ТоварыНеВыданоПриИзменении(Элемент,ТекущийОбъект,ФормаДокумента) Экспорт
	...
	Иначе
		...
		Если ТекущиеДанные.Количество = 0 Тогда           
			...
			Если ИдентификаторТекущейСтроки = ИдентификаторМаксимальнойСтроки Тогда
			    ТекущиеДанные.Количество = ТекущиеДанные.КоличествоПотребность;
			Иначе
				ДанныеСтроки = ТекущийОбъект.Товары.НайтиПоидентификатору(ИдентификаторМаксимальнойСтроки);
				ДанныеСтроки.НеВыдано = Ложь;
				ДанныеСтроки.Количество = ДанныеСтроки.КоличествоПотребность;
				Если ЗначениеЗаполнено(ДанныеСтроки.Номенклатура) Тогда
					ДанныеСтроки.Цена 	= ЦенообразованиеСерверПереопределяемый.ПолучитьЦену(ДанныеСтроки.Номенклатура,ТекущийОбъект.Организация,ТекущийОбъект.Дата);
...
Процедура ВыдачаСИЗ_ТоварыКоличествоПриИзменении(Элемент,ТекущийОбъект,ФормаДокумента) Экспорт
	...
	Если НЕ ТекущийОбъект.ВидВыдачиСИЗ = ПредопределенноеЗначение("Перечисление.ВидыВыдачиСИЗ.ПроизвольнаяВыдача") Тогда
		...
		Если ТекущиеДанные.КоличествоПотребность = 0 Тогда
		    КоличествоКВыдаче = ТекущееКоличество;
		Иначе
			КоличествоКВыдаче = ПроцедурыРаботыСНормамиСервер.ПолучитьКоличествоКВыдачеПоПотребности(ТекущиеДанные.Сотрудник, ТекущиеДанные.НормаВыдачи, ТекущиеДанные.НоменклатураНормы,
										ТекущиеДанные.ДатаВыдачи, ТекущиеДанные.КоличествоПотребность, ПроцедурыРаботыСНормамиСервер.ПолучитьГраницуАнализаПоДокументу(ТекущийОбъект.Ссылка));
		КонецЕсли;
										
		Если КоличествоКВыдаче < ТекущееКоличество Тогда
			ТекущиеДанные.Количество = КоличествоКВыдаче;
		Иначе
...
Процедура СписаниеСИЗ_УправлениеДоступностью(ФормаДокумента) Экспорт
	...
	ФормаДокумента.Элементы.ТоварыКоличество.Доступность = Истина;
	ФормаДокумента.Элементы.ТоварыКоличество.ТолькоПросмотр = Ложь;
