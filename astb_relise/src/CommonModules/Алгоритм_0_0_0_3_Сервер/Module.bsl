...
Процедура ЗачетУпрощеннойВыдачи_ЗаполнитьТаблицуДокумента(ТекущийОбъект, ВидЗаполнения, ЗначениеПараметраЗаполнения) Экспорт
	...
	Если ВидЗаполнения = "Полностью" Тогда
		...
		|	ВЫБОР
		|		КОГДА СрокиНоскиПоУпрощеннойВыдаче.Сотрудник.Владелец.ДискретностьРасчетаПроцентаИзноса = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.День)
		|			ТОГДА РАЗНОСТЬДАТ(СрокиНоскиПоУпрощеннойВыдаче.ДатаВыдачи, &Период, ДЕНЬ) / ВЫБОР
		|					КОГДА СрокиНоскиПоУпрощеннойВыдаче.ПериодичностьВыдачи.ТипПериода = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.Год)
		|						ТОГДА СрокиНоскиПоУпрощеннойВыдаче.ПериодичностьВыдачи.КоличествоПериодов * 365
		|					ИНАЧЕ СрокиНоскиПоУпрощеннойВыдаче.ПериодичностьВыдачи.КоличествоПериодов * 30
		|				КОНЕЦ
		|		ИНАЧЕ РАЗНОСТЬДАТ(СрокиНоскиПоУпрощеннойВыдаче.ДатаВыдачи, &Период, МЕСЯЦ) / ВЫБОР
		|				КОГДА СрокиНоскиПоУпрощеннойВыдаче.ПериодичностьВыдачи.ТипПериода = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.Год)
		|					ТОГДА СрокиНоскиПоУпрощеннойВыдаче.ПериодичностьВыдачи.КоличествоПериодов * 12
		|				ИНАЧЕ СрокиНоскиПоУпрощеннойВыдаче.ПериодичностьВыдачи.КоличествоПериодов
		|			КОНЕЦ
		|	КОНЕЦ * 100 КАК ПроцентИзноса
...
Процедура ПолучитьТаблицуДвиженийВыдачиПоПотребности(ДокументСсылка,ТаблицаДвижений,ДвиженияПоПотребности,ДвиженияПоВыданнымСИЗ,ТаблицаВыдачаПоПотребности) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеСредствЗащитыСотрудника") Тогда
		...
		Иначе
			...
			|			И ВТ_ВыдачаПотребностьРасход.ПроцентИзноса = ВТ_ВыдачаВыдачаРасход.ПроцентИзноса
			|
			|УПОРЯДОЧИТЬ ПО
			|	Сотрудник,
			|	НормаВыдачи,
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриемНаРаботу") 
		...
		|		ВыдачаПоПотребностиОбороты.НормаВыдачи,
		|		ВыдачаПоПотребностиОбороты.НоменклатураНормы,
		|		ВыдачаПоПотребностиОбороты.ДатаВыдачи) КАК ВложенныйЗапрос
		...
		|			И ВложенныйЗапрос.ДатаВыдачи = ВыдачаПоПотребностиОбороты.ДатаВыдачи
		|ГДЕ
		|	НЕ ВыдачаПоПотребностиОбороты.КоличествоПриход = 0
		|	И НЕ НАЧАЛОПЕРИОДА(ВыдачаПоПотребностиОбороты.Период, ДЕНЬ) = ВыдачаПоПотребностиОбороты.ДатаПотребности
		|	И НЕ(ВыдачаПоПотребностиОбороты.Регистратор ССЫЛКА Документ.УстановкаПериодаЗимы
		...
		|	ВЫБОР
		|		КОГДА ВТ_ОстальныеДвижения.ДатаДокумента > ВТ_ОстальныеДвижения.ДатаПотребностиДляРасчетаПроцентаИзноса
		|			ТОГДА 100
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ВЫБОР
		|						КОГДА ВТ_ОстальныеДвижения.Сотрудник.Владелец.ДискретностьРасчетаПроцентаИзноса = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.День)
		|							ТОГДА РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, ДЕНЬ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребностиДляРасчетаПроцентаИзноса, ДЕНЬ)
		|						ИНАЧЕ РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, МЕСЯЦ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребностиДляРасчетаПроцентаИзноса, МЕСЯЦ)
		|					КОНЕЦ >= 1
		|					ТОГДА 100
		|				ИНАЧЕ ВЫБОР
		|						КОГДА ВТ_ОстальныеДвижения.ПроцентИзноса = 0
		|							ТОГДА ВЫБОР
		|									КОГДА ВТ_ОстальныеДвижения.Сотрудник.Владелец.ДискретностьРасчетаПроцентаИзноса = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.День)
		|										ТОГДА РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, ДЕНЬ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребностиДляРасчетаПроцентаИзноса, ДЕНЬ)
		|									ИНАЧЕ РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, МЕСЯЦ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребностиДляРасчетаПроцентаИзноса, МЕСЯЦ)
		...
		|									ТОГДА РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, ДЕНЬ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребностиДляРасчетаПроцентаИзноса, ДЕНЬ)
		|								ИНАЧЕ РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, МЕСЯЦ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребностиДляРасчетаПроцентаИзноса, МЕСЯЦ)
		|							КОНЕЦ * (1 - ВТ_ОстальныеДвижения.ПроцентИзноса / 100) * 100 + ВТ_ОстальныеДвижения.ПроцентИзноса
		|					КОНЕЦ
		|			КОНЕЦ
		|	КОНЕЦ
		|ИЗ
...
Процедура УточнитьПотребностьПоУстановкеПериодаЗимы(ТаблицаЗапроса,ТаблицаЗимы,Организация,ДатаАнализа,МассивСотрудников,ДатаДокумента)
	...
	|			ТОГДА ДОБАВИТЬКДАТЕ(ЕСТЬNULL(ВТ_ВыдачаПоПотребности.ДатаВыдачи, ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)), МЕСЯЦ, ВТ_СоставНормы.КоличествоПериодов * 12)
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(ЕСТЬNULL(ВТ_ВыдачаПоПотребности.ДатаВыдачи, ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)), МЕСЯЦ, ВТ_СоставНормы.КоличествоПериодов)
	|	КОНЕЦ КАК ДатаСледующейВыдачи,
	...
	//АсТБ_Alexey_76619_********************************************************************
	|	ВТ_ТаблицаПотребности.ЭтоЗима КАК ЭтоЗима,
	|	ВТ_ПотребностьСДатойВыдачи.Количество КАК КоличествоПотребность,
	|	ВТ_ТаблицаЗимы.ДатаНачалаЗимы КАК ДатаНачалаЗимы,
	|	ВТ_ТаблицаЗимы.ДатаОкончанияЗимы КАК ДатаОкончанияЗимы,
...
Процедура СкорректироватьТаблицуКонечнойПотребности(ТаблицаЗапроса,Организация,ДатаАнализа,МассивСотрудников,ТаблицаВыдачаПоПотребности = Неопределено)
	...
	|	ВыдачаПоПотребности.ПроцентИзноса КАК ПроцентИзноса,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ВыдачаПоПотребности.Регистратор ССЫЛКА Документ.КорректировкаРегистров
	|				ТОГДА ВЫБОР
	|						КОГДА ПОДСТРОКА(ВыдачаПоПотребности.Регистратор.Комментарий, 1, 23) = ""#РасчетПоДатеДокумента#""
	|							ТОГДА ВыдачаПоПотребности.Период
	|						ИНАЧЕ ВыдачаПоПотребности.ДатаВыдачи
	|					КОНЕЦ
	|			ИНАЧЕ ВыдачаПоПотребности.Период
	|		КОНЕЦ) КАК ДатаДокумента
	|ПОМЕСТИТЬ ВТ_ДатыДокментовБезЗачетаВыданныхСредств
	|ИЗ
	|	РегистрНакопления.ВыдачаПоПотребности КАК ВыдачаПоПотребности
	|ГДЕ
	|	ВыдачаПоПотребности.Сотрудник В(&МассивСотрудников)
	|	И ВыдачаПоПотребности.Организация = &Организация
	|	И НЕ ВыдачаПоПотребности.Количество = 0
	|	И НЕ НАЧАЛОПЕРИОДА(ВыдачаПоПотребности.Период, ДЕНЬ) = ВыдачаПоПотребности.ДатаПотребности
	|	И НЕ(ВыдачаПоПотребности.Регистратор ССЫЛКА Документ.УстановкаПериодаЗимы
	|				ИЛИ ВыдачаПоПотребности.Регистратор ССЫЛКА Документ.ОтсутствиеНаРабочемМесте
	|				ИЛИ ВыдачаПоПотребности.Регистратор ССЫЛКА Документ.ЗачетВыданныхСредствЗащиты
	|				ИЛИ ВыдачаПоПотребности.Регистратор ССЫЛКА Документ.ПереносЗимнейПотребности)
	|
	|СГРУППИРОВАТЬ ПО
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	ВЫБОР 
	|		КОГДА ВыдачаПоПотребностиОбороты.Регистратор ССЫЛКА Документ.ЗачетВыданныхСредствЗащиты
	|			ТОГДА ВЫБОР
	|					КОГДА ВТ_СоставНормы.ПериодичностьВыдачиТипПериода = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.Год)
	|						ТОГДА ДОБАВИТЬКДАТЕ(ВыдачаПоПотребностиОбороты.ДатаВыдачи, МЕСЯЦ, ВТ_СоставНормы.ПериодичностьВыдачиКоличествоПериодов * 12)
	|					ИНАЧЕ ДОБАВИТЬКДАТЕ(ВыдачаПоПотребностиОбороты.ДатаВыдачи, МЕСЯЦ, ВТ_СоставНормы.ПериодичностьВыдачиКоличествоПериодов)
	|				КОНЕЦ
	|		ИНАЧЕ ВыдачаПоПотребностиОбороты.ДатаПотребности
	|	КОНЕЦ КАК ДатаПотребностиСкорректированная,
	|	ВыдачаПоПотребностиОбороты.Регистратор КАК Регистратор,
	//---АСТБ_Горюшин_Алексей_53006
	|	ВыдачаПоПотребностиОбороты.ДатаПотребности КАК ДатаПотребности,
	|	ВыдачаПоПотребностиОбороты.ДатаВыдачи КАК ДатаВыдачи,
	|	ВыдачаПоПотребностиОбороты.ПроцентИзноса КАК ПроцентИзноса,
	...
	|		КОНЕЦ) КАК ДатаДокумента
	//+++АСТБ_Горюшин_Алексей_53006
	|ПОМЕСТИТЬ ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки
	//было
	//|ПОМЕСТИТЬ ВТ_ПоследнийПриходВыдачиПоПотребности
	//---АСТБ_Горюшин_Алексей_53006
	|ИЗ
	...
	//+++АСТБ_Горюшин_Алексей_53006			
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СоставНормы КАК ВТ_СоставНормы
	|		ПО ВыдачаПоПотребностиОбороты.НормаВыдачи = ВТ_СоставНормы.Ссылка
	|			И ВыдачаПоПотребностиОбороты.НоменклатураНормы = ВТ_СоставНормы.НоменклатураНормы
	//---АСТБ_Горюшин_Алексей_53006
	|ГДЕ
	|	НЕ ВыдачаПоПотребностиОбороты.КоличествоПриход = 0
	|	И НЕ НАЧАЛОПЕРИОДА(ВыдачаПоПотребностиОбороты.Период, ДЕНЬ) = ВыдачаПоПотребностиОбороты.ДатаПотребности
	...
	|	ВыдачаПоПотребностиОбороты.ПроцентИзноса
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВыдачаПоПотребностиОбороты.Регистратор,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.Сотрудник КАК Сотрудник,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.НормаВыдачи КАК НормаВыдачи,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.НоменклатураНормы КАК НоменклатураНормы,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.ДатаПотребностиСкорректированная КАК ДатаПотребностиСкорректированная,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.ДатаПотребности КАК ДатаПотребности,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.ДатаВыдачи КАК ДатаВыдачи,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.ПроцентИзноса КАК ПроцентИзноса,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.ДатаДокумента КАК ДатаДокумента,
	|	ВТ_ДатыДокментовБезЗачетаВыданныхСредств.ДатаДокумента КАК ДатаДокументаДоЗачетаВыдачи
	|ПОМЕСТИТЬ ВТ_ПоследнийПриходВыдачиПоПотребности
	|ИЗ
	|	ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки КАК ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДатыДокментовБезЗачетаВыданныхСредств КАК ВТ_ДатыДокментовБезЗачетаВыданныхСредств
	|		ПО ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.Сотрудник = ВТ_ДатыДокментовБезЗачетаВыданныхСредств.Сотрудник
	|			И ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.ДатаВыдачи = ВТ_ДатыДокментовБезЗачетаВыданныхСредств.ДатаВыдачи
	|			И ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.ПроцентИзноса = ВТ_ДатыДокментовБезЗачетаВыданныхСредств.ПроцентИзноса
	|			И (ВТ_ПоследнийПриходВыдачиПоПотребности_ДоКорректировки.Регистратор ССЫЛКА Документ.ЗачетВыданныхСредствЗащиты)
	|;
	//---АСТБ_Горюшин_Алексей_53006
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВТ_ВыдачаПоПотребности.Количество КАК Количество
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВТ_ПоследнийПриходВыдачиПоПотребности.ДатаПотребностиСкорректированная КАК ДатаПотребностиСкорректированная,
	|	ВТ_ПоследнийПриходВыдачиПоПотребности.ДатаДокументаДоЗачетаВыдачи КАК ДатаДокументаДоЗачетаВыдачи
	//---АСТБ_Горюшин_Алексей_53006
	|ПОМЕСТИТЬ ВТ_АктуальнаяВыдачаПоПотребности
	|ИЗ
	|	ВТ_ВыдачаПоПотребности КАК ВТ_ВыдачаПоПотребности
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	ВТ_АктуальнаяВыдачаПоПотребности.ДатаПотребностиСкорректированная КАК ДатаПотребностиСкорректированная,
	|	ВТ_АктуальнаяВыдачаПоПотребности.ДатаДокументаДоЗачетаВыдачи КАК ДатаДокументаДоЗачетаВыдачи,
	//---АСТБ_Горюшин_Алексей_53006
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВТ_АктуальнаяВыдачаПоПотребностиБезДатыПотребности.ДатаПотребностиСкорректированная КАК ДатаПотребностиСкорректированная,
	|	ВТ_АктуальнаяВыдачаПоПотребностиБезДатыПотребности.ДатаДокументаДоЗачетаВыдачи КАК ДатаДокументаДоЗачетаВыдачи
	//---АСТБ_Горюшин_Алексей_53006
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВТ_АктуальнаяВыдачаПоПотребностиСДатойПотребности.ДатаПотребностиСкорректированная КАК ДатаПотребностиСкорректированная,
	|	ВТ_АктуальнаяВыдачаПоПотребностиСДатойПотребности.ДатаДокументаДоЗачетаВыдачи КАК ДатаДокументаДоЗачетаВыдачи
	//---АСТБ_Горюшин_Алексей_53006
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВТ_АктуальнаяВыдачаПоПотребностиСРасчетнойДатойПотребности.ДатаПотребностиСкорректированная КАК ДатаПотребностиСкорректированная,
	|	ВТ_АктуальнаяВыдачаПоПотребностиСРасчетнойДатойПотребности.ДатаДокументаДоЗачетаВыдачи КАК ДатаДокументаДоЗачетаВыдачи
	//---АСТБ_Горюшин_Алексей_53006
	|ИЗ
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВТ_СобраннаяАктуальнаяВыдачаПоПотребности.ДатаПотребностиСкорректированная КАК ДатаПотребностиСкорректированная,
	|	ВТ_СобраннаяАктуальнаяВыдачаПоПотребности.ДатаДокументаДоЗачетаВыдачи КАК ДатаДокументаДоЗачетаВыдачи
	//---АСТБ_Горюшин_Алексей_53006
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВТ_СобраннаяАктуальнаяВыдачаПоПотребности.ДатаПотребностиСкорректированная,
	|	ВТ_СобраннаяАктуальнаяВыдачаПоПотребности.ДатаДокументаДоЗачетаВыдачи
	//---АСТБ_Горюшин_Алексей_53006
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	ЕСТЬNULL(ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокументаДоЗачетаВыдачи, ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокумента) КАК ДатаДокумента,
	//было
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	И ВЫБОР
	|			КОГДА ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.Сотрудник.Владелец.ДискретностьРасчетаПроцентаИзноса = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.День)
	|				ТОГДА НЕ РАЗНОСТЬДАТ(ЕСТЬNULL(ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокументаДоЗачетаВыдачи, ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокумента), ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаПотребности, ДЕНЬ) = 0
	|			ИНАЧЕ НЕ РАЗНОСТЬДАТ(ЕСТЬNULL(ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокументаДоЗачетаВыдачи, ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокумента), ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаПотребности, МЕСЯЦ) = 0
	|		КОНЕЦ
	//было
	//|	И ВЫБОР
	//|			КОГДА ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.Сотрудник.Владелец.ДискретностьРасчетаПроцентаИзноса = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.День)
	//|				ТОГДА НЕ РАЗНОСТЬДАТ(ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокумента, ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаПотребности, ДЕНЬ) = 0
	//|			ИНАЧЕ НЕ РАЗНОСТЬДАТ(ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокумента, ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаПотребности, МЕСЯЦ) = 0
	//|		КОНЕЦ
	//---АСТБ_Горюшин_Алексей_53006
	|;
	|
	...
	|	ВТ_ТаблицаКонечнойПотребности.ДатаОкончанияОтсутствияНаРабочемМесте КАК ДатаОкончанияОтсутствияНаРабочемМесте
	//+++АСТБ_Горюшин_Алексей_53006
	|	,ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаПотребностиСкорректированная,
	|	ВТ_СгруппированнаяАктуальнаяВыдачаПоПотребности.ДатаДокументаДоЗачетаВыдачи
	//---АСТБ_Горюшин_Алексей_53006
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	ЕСТЬNULL(ВТ_ПотребностьИВыдача.ДатаДокументаДоЗачетаВыдачи, ВТ_ПотребностьИВыдача.ДатаДокумента) КАК ДатаДокумента,
	//было
	...
	//+++АСТБ_Горюшин_Алексей_53006
	|	ЕСТЬNULL(ВТ_ПотребностьИВыдача.ДатаДокументаДоЗачетаВыдачи, ВТ_ПотребностьИВыдача.ДатаДокумента),
	//было
	...
	|	ВЫБОР
	|		КОГДА ВТ_ВыдачаБезПотребности.ДатаДокумента > ВТ_ВыдачаБезПотребности.ДатаПотребности
	|			ТОГДА 100
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВЫБОР
	|						КОГДА ВТ_ВыдачаБезПотребности.Сотрудник.Владелец.ДискретностьРасчетаПроцентаИзноса = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.День)
	|							ТОГДА РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, &Период, ДЕНЬ) / РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, ВТ_ВыдачаБезПотребности.ДатаПотребности, ДЕНЬ)
	|						ИНАЧЕ РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, &Период, МЕСЯЦ) / РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, ВТ_ВыдачаБезПотребности.ДатаПотребности, МЕСЯЦ)
	|					КОНЕЦ >= 1
	|					ТОГДА 100
	|				ИНАЧЕ ВЫБОР
	|						КОГДА ВТ_ВыдачаБезПотребности.ПроцентИзноса = 0
	|							ТОГДА ВЫБОР
	|									КОГДА ВТ_ВыдачаБезПотребности.Сотрудник.Владелец.ДискретностьРасчетаПроцентаИзноса = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.День)
	|										ТОГДА РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, &Период, ДЕНЬ) / РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, ВТ_ВыдачаБезПотребности.ДатаПотребности, ДЕНЬ)
	|									ИНАЧЕ РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, &Период, МЕСЯЦ) / РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, ВТ_ВыдачаБезПотребности.ДатаПотребности, МЕСЯЦ)
	...
	|									ТОГДА РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, &Период, ДЕНЬ) / РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, ВТ_ВыдачаБезПотребности.ДатаПотребности, ДЕНЬ)
	|								ИНАЧЕ РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, &Период, МЕСЯЦ) / РАЗНОСТЬДАТ(ВТ_ВыдачаБезПотребности.ДатаДокумента, ВТ_ВыдачаБезПотребности.ДатаПотребности, МЕСЯЦ)
	|							КОНЕЦ * (1 - ВТ_ВыдачаБезПотребности.ПроцентИзноса / 100) * 100 + ВТ_ВыдачаБезПотребности.ПроцентИзноса
	|					КОНЕЦ
	|			КОНЕЦ
	|	КОНЕЦ КАК НовыйПроцентИзноса,
	...
	//+++АСТБ_Горюшин_Алексей_53006
	ТаблицаПотребности							= Результат[17].Выгрузить();
	ТаблицаВыдачиБезПотребности 				= Результат[18].Выгрузить();
	ТаблицаДляПоискаПотребности 				= Результат[19].Выгрузить();
	ТаблицаРазличныхДатВыдачиПоДатеПотребности 	= Результат[20].Выгрузить();
	//было
	...
	//+++АСТБ_Горюшин_Алексей_53006
	ИсходнаяТаблицаВыдачиБезПотребности = Результат[18].Выгрузить();
	//было
...
Процедура ВыдачаСредствЗащитыСотруднику_ПолучитьТаблицуДвиженийПоВыданнымСИЗ(ДокументСсылка,ТаблицаДвижений) Экспорт
	...
	//Танцюра А.Н. -- №133496 Возможность проставлять процент износа в документе выдачи для комплектующих -- 13.10.2021 >>>
	//***Трегубов А.А.*** -- исправление ошибки в 115 релизе --  07.10.2021 <<<	
		...
		Для Каждого НайденнаяСтрокаДокумента Из НайденныеСтрокиДокумента Цикл
			...
			Для Каждого НайденнаяСтрокаПотребности Из НайденныеСтрокиПотребности Цикл
				...
				НоваяДатаПотребности = ?(НайденнаяСтрокаПотребности.ПериодичностьВыдачи.ТипПериода = Перечисления.ДоступныеПериодыОтчета.Год,
										ДобавитьМесяц(СтрокаТаблицыВыдачи.ДатаВыдачи,НайденнаяСтрокаПотребности.ПериодичностьВыдачи.КоличествоПериодов * 12),
										ДобавитьМесяц(СтрокаТаблицыВыдачи.ДатаВыдачи,НайденнаяСтрокаПотребности.ПериодичностьВыдачи.КоличествоПериодов));
