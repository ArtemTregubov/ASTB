...
Процедура УстановитьНастройкуКаталогСбораИнформацииОбОбменах(СтруктураНастроекОбмена)
	...
	ПараметрЗапуска = "";
	Попытка
		ПараметрЗапуска = ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить("ПараметрЗапуска");
	Исключение
		// Параметр может быть не инициализирован - это не ошибка.
		Возврат;
	КонецПопытки;
	Если ЗначениеЗаполнено(ПараметрЗапуска) Тогда
		СтруктураПараметровЗапуска = СтроковыеФункцииКлиентСервер.ПолучитьПараметрыИзСтроки(ПараметрЗапуска);
