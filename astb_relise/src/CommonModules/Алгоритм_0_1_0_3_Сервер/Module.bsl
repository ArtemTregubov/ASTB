...
Процедура ПолучитьТаблицуДвиженийВыдачиПоПотребности(ДокументСсылка,ТаблицаДвижений,ДвиженияПоПотребности,ДвиженияПоВыданнымСИЗ,ТаблицаВыдачаПоПотребности) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПриемНаРаботу") 
		...
		|	&ТаблицаДвиженийПоПотребности КАК ТаблицаДвиженийПоПотребности
		|ГДЕ
		|	ТаблицаДвиженийПоПотребности.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТ_ДвиженияПоПотребности КАК ВТ_ДвиженияПоПотребности
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВыдачаПоПотребности КАК ВТ_ВыдачаПоПотребности
		|		ПО ВТ_ДвиженияПоПотребности.Сотрудник = ВТ_ВыдачаПоПотребности.Сотрудник
		|			И ВТ_ДвиженияПоПотребности.НормаВыдачи = ВТ_ВыдачаПоПотребности.НормаВыдачи
		|			И ВТ_ДвиженияПоПотребности.НоменклатураНормы = ВТ_ВыдачаПоПотребности.НоменклатураНормы
		|			И ВТ_ДвиженияПоПотребности.ДатаПотребности = ВТ_ВыдачаПоПотребности.ДатаПотребности
		|			И ВТ_ДвиженияПоПотребности.ПроцентИзноса = ВТ_ВыдачаПоПотребности.ПроцентИзноса
		|ГДЕ
		...
		|	ВТ_ПоследнийПриходВыдачиПоПотребности.Сотрудник КАК Сотрудник,
		...
		|			И ВТ_ВыдачаПоПотребностиРасход.Количество = ВТ_ТаблицаВыдачаПоПотребности.Количество
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПоследнийПриходВыдачиПоПотребности КАК ВТ_ПоследнийПриходВыдачиПоПотребности
		...
		|	И НЕ ВТ_ПоследнийПриходВыдачиПоПотребности.Сотрудник ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА ВТ_ОстальныеДвижения.ДатаДокумента > ВТ_ОстальныеДвижения.ДатаПотребности
		|			ТОГДА 100
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ВЫБОР
		...
		|							ТОГДА РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, ДЕНЬ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребности, ДЕНЬ)
		|						ИНАЧЕ РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, &Период, МЕСЯЦ) / РАЗНОСТЬДАТ(ВТ_ОстальныеДвижения.ДатаДокумента, ВТ_ОстальныеДвижения.ДатаПотребности, МЕСЯЦ)
		|					КОНЕЦ >= 1
		|					ТОГДА 100
		|				ИНАЧЕ ВЫБОР
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УстановкаПериодаЗимы") 
		...
		|	ВТ_ДвиженияПоПотребности.НормаВыдачи КАК НормаВыдачи,
		|	ВТ_ДвиженияПоПотребности.НоменклатураНормы КАК НоменклатураНормы,
		|	НАЧАЛОПЕРИОДА(ВТ_ДвиженияПоПотребности.ДатаПотребности, ДЕНЬ) КАК ДатаПотребности,
		...
		Результат = Запрос.ВыполнитьПакет();
		ТаблицаРасхода = Результат[4].Выгрузить();
		ТаблицаПрихода = Результат[5].Выгрузить();
...
Функция ПолучитьИсходнуюПотребность(СтруктураОтбора) Экспорт
	...
	|	ЕСТЬNULL(ВТ_ВыдачаПоПотребности.КоличествоВыдано, 0) КАК КоличествоВыдано
	|ПОМЕСТИТЬ ВТ_Результат
	|ИЗ
	...
	|	ВТ_Результат.НоменклатураНормы КАК НоменклатураНормы,
	|	ВТ_Результат.ДатаПотребности КАК ДатаПотребности,
	|	МАКСИМУМ(0) КАК ПроцентИзноса,
	|	МАКСИМУМ(ВТ_Результат.КоличествоПотребность) КАК КоличествоПотребность,
...
Функция ПолучитьКонечнуюПотребностьПоДокументуПриемаНаРаботу(СтруктураОтбора,ТаблицаВыдачаПоПотребности) Экспорт
	...
	СкорректироватьТаблицуВыдачиПоПотребностиПоЗиме(ТаблицаЗапроса,ТаблицаВыдачаПоПотребности);
...
Процедура УточнитьПотребностьПоУстановкеПериодаЗимы(ТаблицаЗапроса,ТаблицаЗимы,Организация,ДатаАнализа,МассивСотрудников,ДатаДокумента)
	...
	|	КОНЕЦ КАК ДатаПотребности,
	|	ВТ_ТаблицаПотребности.ЭтоЗима КАК ЭтоЗима,
	|	СУММА(ВТ_ПотребностьСДатойВыдачи.Количество) КАК КоличествоПотребность,
	|	ВТ_ТаблицаЗимы.ДатаНачалаЗимы КАК ДатаНачалаЗимы,
	|	ВТ_ТаблицаЗимы.ДатаОкончанияЗимы КАК ДатаОкончанияЗимы,
	...
	|			КОНЕЦ
	|	КОНЕЦ,
	|	ВТ_ТаблицаПотребности.ЭтоЗима,
	|	ВТ_ТаблицаЗимы.ДатаНачалаЗимы,
	|	ВТ_ТаблицаЗимы.ДатаОкончанияЗимы,
	|	ВТ_ПотребностьСДатойВыдачи.ПроцентИзноса
	|
	|УПОРЯДОЧИТЬ ПО
	|	Сотрудник,
	|	НормаВыдачи,
...
Процедура СкорректироватьТаблицуВыдачиПоПотребностиПоЗиме(ТаблицаЗапроса,ТаблицаВыдачаПоПотребности)
...
Процедура СкорректироватьТаблицуКонечнойПотребности(ТаблицаЗапроса,Организация,ДатаАнализа,МассивСотрудников,ТаблицаВыдачаПоПотребности = Неопределено)
	...
	|	ЕСТЬNULL(ВТ_ВыдачаПоПотребности.ДатаВыдачи, ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)) КАК ДатаВыдачи,
	|	ЕСТЬNULL(ВТ_ВыдачаПоПотребности.ПроцентИзноса, 0) КАК ПроцентИзноса,
	|	ЕСТЬNULL(ВТ_ВыдачаПоПотребности.Количество, 0) КАК КоличествоВыдано,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВыдачаПоПотребности КАК ВТ_ВыдачаПоПотребности
	|		ПО ВТ_ТаблицаКонечнойПотребности.Сотрудник = ВТ_ВыдачаПоПотребности.Сотрудник
	|			И ВТ_ТаблицаКонечнойПотребности.НормаВыдачи = ВТ_ВыдачаПоПотребности.НормаВыдачи
	|			И ВТ_ТаблицаКонечнойПотребности.НоменклатураНормы = ВТ_ВыдачаПоПотребности.НоменклатураНормы
	|			И ВТ_ТаблицаКонечнойПотребности.ДатаПотребности = ВТ_ВыдачаПоПотребности.ДатаПотребности
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СоставНормы КАК ВТ_СоставНормы
	|		ПО ВТ_ТаблицаКонечнойПотребности.НормаВыдачи = ВТ_СоставНормы.Ссылка
	|			И ВТ_ТаблицаКонечнойПотребности.НоменклатураНормы = ВТ_СоставНормы.НоменклатураНормы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Сотрудник,
	|	НормаВыдачи,
	...
	|			ТОГДА ДОБАВИТЬКДАТЕ(ВТ_ВыдачаПоПотребности.ДатаВыдачи, ДЕНЬ, ВТ_СоставНормы.ПериодичностьВыдачиКоличествоПериодов * 365 * (1 - ВТ_ВыдачаПоПотребности.ПроцентИзноса / 100))
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(ВТ_ВыдачаПоПотребности.ДатаВыдачи, ДЕНЬ, ВТ_СоставНормы.ПериодичностьВыдачиКоличествоПериодов * 30 * (1 - ВТ_ВыдачаПоПотребности.ПроцентИзноса / 100))
	...
	|	ВТ_ВыдачаПоПотребности КАК ВТ_ВыдачаПоПотребности
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВыданныеСИЗ КАК ВТ_ВыданныеСИЗ
	|		ПО ВТ_ВыдачаПоПотребности.Сотрудник = ВТ_ВыданныеСИЗ.Сотрудник
	|			И ВТ_ВыдачаПоПотребности.НоменклатураНормы = ВТ_ВыданныеСИЗ.НоменклатураНормы
	|			И ВТ_ВыдачаПоПотребности.ДатаВыдачи = ВТ_ВыданныеСИЗ.ДатаВыдачи
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаКонечнойПотребности КАК ВТ_ТаблицаКонечнойПотребности
	|		ПО ВТ_ВыдачаПоПотребности.Сотрудник = ВТ_ТаблицаКонечнойПотребности.Сотрудник
	|			И ВТ_ВыдачаПоПотребности.НормаВыдачи = ВТ_ТаблицаКонечнойПотребности.НормаВыдачи
	|			И ВТ_ВыдачаПоПотребности.НоменклатураНормы = ВТ_ТаблицаКонечнойПотребности.НоменклатураНормы
	|			И ВТ_ВыдачаПоПотребности.ДатаПотребности = ВТ_ТаблицаКонечнойПотребности.ДатаПотребности
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СоставНормы КАК ВТ_СоставНормы
	|		ПО ВТ_ВыдачаПоПотребности.НормаВыдачи = ВТ_СоставНормы.Ссылка
	...
	|	И НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ) < НАЧАЛОПЕРИОДА(ВЫБОР
	|				КОГДА ВТ_СоставНормы.ПериодичностьВыдачиТипПериода = ЗНАЧЕНИЕ(Перечисление.ДоступныеПериодыОтчета.Год)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ВТ_ВыдачаПоПотребности.ДатаВыдачи, ДЕНЬ, ВТ_СоставНормы.ПериодичностьВыдачиКоличествоПериодов * 365 * (1 - ВТ_ВыдачаПоПотребности.ПроцентИзноса / 100))
	|				ИНАЧЕ ДОБАВИТЬКДАТЕ(ВТ_ВыдачаПоПотребности.ДатаВыдачи, ДЕНЬ, ВТ_СоставНормы.ПериодичностьВыдачиКоличествоПериодов * 30.42 * (1 - ВТ_ВыдачаПоПотребности.ПроцентИзноса / 100))
	|			КОНЕЦ, МЕСЯЦ)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Сотрудник,
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВТ_ТаблицаКонечнойПотребности.Сотрудник,
	|	ВТ_ТаблицаКонечнойПотребности.НоменклатураНормы";
	...
	ТаблицаПотребности 			= Результат[4].Выгрузить();
	ТаблицаВыдачиБезПотребности = Результат[5].Выгрузить();
	ТаблицаДляПоискаПотребности = Результат[6].Выгрузить();
		...
		Для Каждого НайденнаяСтрокаПотребности Из НайденныеСтрокиПотребности Цикл
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если НайденнаяСтрока.Количество < ОстатокПоПотребности Тогда
					...
					НоваяСтрока.ДатаПотребности 		= НайденнаяСтрока.РасчетнаяДатаПотребности;
					НоваяСтрока.ПроцентИзноса 			= НайденнаяСтрока.ПроцентИзноса;
					...
					ВыдачаПоПотребности_НоваяСтрока.ДатаПотребности 	= НайденнаяСтрока.РасчетнаяДатаПотребности;
					...
					ВыдачаПоПотребности_НоваяСтрока.ПроцентИзноса 		= НайденнаяСтрока.ПроцентИзноса;
	...
	ИсходнаяТаблицаВыдачиБезПотребности = Результат[5].Выгрузить();
	...
	|	ВТ_ИсходнаяТаблицаВыдачиБезПотребности.ДатаПотребности КАК ДатаПотребности,
	...
	|ГДЕ
	|	НЕ ВТ_ИсходнаяТаблицаВыдачиБезПотребности.Количество = ВТ_ТаблицаВыдачиБезПотребности.Количество
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	НЕ ВТ_ИсходнаяТаблицаВыдачиБезПотребности.Количество = ВТ_ТаблицаВыдачиБезПотребности.Количество
	|	И НЕ ВТ_ТаблицаВыдачиБезПотребности.Количество = 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
